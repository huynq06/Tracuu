...
namespace Web.Portal.DataBase
{
    public class OracleHelper
    {
            ...
            /// Thuanld changed 16/04/2013 : if ((p.Direction == ParameterDirection.InputOutput) && (p.Value == null)) --> if ((p.Value == null)) 
            ...
            private static void AssignParameterValues(OracleParameter[] commandParameters, object[] parameterValues)
            {
                ...
                for (int i = 0, j = commandParameters.Length; i < j; i++)
                {
                    ...
                    //commandParameters[i].Value = string.IsNullOrEmpty(parameterValues[i].ToString().Trim()) ? "  " : parameterValues[i];
                }
            }


            private static void AssignParameterValues(OracleParameter[] commandParameters, string OutValue, object[] parameterValues)
            {
                if ((commandParameters == null) || (parameterValues == null))
                {
                    //do nothing if we get no data 
                ...
                int i = 0;
                int j = 0;
                for (i = 0, j = commandParameters.Length; i < j - 1; i++)
                {
                    commandParameters[i].Value = parameterValues[i];
                    // commandParameters[i].Value = string.IsNullOrEmpty(parameterValues[i].ToString().Trim()) ? "  " : parameterValues[i];
                }
                ...
                op = new OracleParameter(OutValue, OracleDbType.Int32);
                op.Direction = ParameterDirection.Output;
                op.Size = 5;
                commandParameters[i] = op;
            ...
            public static int ExecuteNonQuery(string connectionString, CommandType commandType, string commandText, string OutValue, params OracleParameter[] commandParameters)
            {
                //create & open an OracleConnection, and dispose of it after we are done. 
                using (OracleConnection cn = new OracleConnection(connectionString))
                {
                    ...
                    //call the overload that takes a connection in place of the connection string 
                    return ExecuteNonQuery(cn, commandType, commandText, OutValue, commandParameters);
                }
            }
            /// <summary> 
            ...
            public static int ExecuteNonQuery(string connectionString, string spName, string OutValue, params object[] parameterValues)
            {
                //if we got parameter values, we need to figure out where they go 
                if ((parameterValues != null) && (parameterValues.Length > 0))
                {
                    ...
                    //assign the provided values to these parameters based on parameter order 
                    AssignParameterValues(commandParameters, OutValue, parameterValues);

                    //call the overload that takes an array of OracleParameters 
                    return ExecuteNonQuery(connectionString, CommandType.StoredProcedure, spName, OutValue, commandParameters);
                }
                //otherwise we can just call the SP without params 
                else
                {
            ...
            public static int ExecuteNonQuery(OracleConnection connection, CommandType commandType, string commandText, string OutValue, params OracleParameter[] commandParameters)
            {
                //create a command and prepare it for execution 
                OracleCommand cmd = new OracleCommand();
                PrepareCommand(cmd, connection, (OracleTransaction)null, commandType, commandText, commandParameters);
                ...
                cmd.ExecuteNonQuery();
                OracleDataReader oReader = cmd.Parameters[OutValue].Value as OracleDataReader;

                int ID = Convert.ToInt32(cmd.Parameters[OutValue].Value.ToString());
            ...
            /// <summary>
            /// Hàm get all dữ liệu với phân trang
            /// </summary>
            /// <param name="connectionString"></param>
            /// <param name="commandType"></param>
            /// <param name="commandText"></param>
            /// <param name="oParamTotalRecordName"></param>
            /// <param name="oParamCursorName"></param>
            ...
            /// <Modified>
            /// Author      Date        Comment
            /// Thuanld     16/04/2013  Tạo mới
            /// </Modified>
            public static DataTable ExecuteNonQuery(string connectionString, CommandType commandType, string commandText,
                string oParamTotalRecordName, string oParamCursorName, string oParamOrderName, string oParamOrderValue, string oParamSortFieldName,
                ...
                string oParamPageName, int page, out int totalRecords)
            {
                using (OracleConnection cn = new OracleConnection(connectionString))
                {
                    try
                    {
                        ...
                        //Params
                        op = new OracleParameter(oParamOrderName, OracleDbType.Varchar2);
                        op.Direction = ParameterDirection.Input;
                        op.Value = oParamOrderValue;

                        cmd.Parameters.Add(op);
                        op = new OracleParameter(oParamSortFieldName, OracleDbType.Varchar2);
                        op.Direction = ParameterDirection.Input;
                        op.Value = oParamSortFieldValue;
                        cmd.Parameters.Add(op);

                        op = new OracleParameter(oParamPageName, OracleDbType.Int32);
                        op.Direction = ParameterDirection.Input;
                        op.Value = page;
                        cmd.Parameters.Add(op);

                        op = new OracleParameter(oParamNumPerPageName, OracleDbType.Int32);
                        op.Direction = ParameterDirection.Input;
                        op.Value = rowsNum;
                        cmd.Parameters.Add(op);

                        op = new OracleParameter(oParamTotalRecordName, OracleDbType.Int32);
                        op.Direction = ParameterDirection.Output;
                        cmd.Parameters.Add(op);

                        op = new OracleParameter(oParamCursorName, OracleDbType.RefCursor);
                        op.Direction = ParameterDirection.Output;
                        cmd.Parameters.Add(op);
                        ...
                        //-Params
                        cmd.ExecuteNonQuery();
                        totalRecords = Convert.ToInt32(cmd.Parameters[4].Value);

                        dr = (OracleDataReader)cmd.Parameters[5].Value;
            ...
            /// <summary>
            /// Hàm get all dữ liệu với phân trang trả
            /// </summary>
            /// <param name="connectionString"></param>
            /// <param name="commandType"></param>
            /// <param name="commandText"></param>
            /// <param name="paramArray">Mảng giá trị các tham số truyền vào (các tham số thứ tự phải giống trong StoreProcedure)</param>
            /// <param name="totalRecords">Tổng số bản ghi theo câu query</param>
            /// <returns>Trả về DataTable và tổng số bản ghi theo câu query</returns>
            /// <Modified>
            /// Author      Date        Comment
            /// Thuanld     16/04/2013  Tạo mới
            /// </Modified>
            public static DataTable ExecuteNonQuery(string connectionString, CommandType commandType, string commandText,
                object[] paramArray, out int totalRecords)
            {
                using (OracleConnection cn = new OracleConnection(connectionString))
                {
                    try
                    {
                        ...
                        int indexOfoParamNumber = -1;
                        int indexOfoParamCursor = -1;
                        if (commandParameters.Length != paramArray.Length)
                            throw new Exception("Wrong number of params in array");
                        for (int i = 0; i < commandParameters.Length; i++)
                        {
                            if (commandParameters[i].Direction == ParameterDirection.Input)
                            {
                                commandParameters[i].Value = paramArray[i] != null ? paramArray[i].ToString() : null;
                            }
                            else
                            {
                                if (commandParameters[i].Direction == ParameterDirection.Output)
                                {
                                    if (commandParameters[i].OracleDbType == OracleDbType.Int32)
                                        indexOfoParamNumber = i;
                                    if (commandParameters[i].OracleDbType == OracleDbType.RefCursor)
                                        indexOfoParamCursor = i;
                                }
                            }
                        ...
                        cmd.ExecuteNonQuery();
                        if (indexOfoParamNumber > -1)
                        {
                            totalRecords = Convert.ToInt32(cmd.Parameters[indexOfoParamNumber].Value);
                        }
                        DataTable dt = new DataTable();
                        if (indexOfoParamCursor > -1)
                        {
                            dr = (OracleDataReader)cmd.Parameters[indexOfoParamCursor].Value;
