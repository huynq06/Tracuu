...
{
    [RoutePrefix("api/FlightImpApi")]
    public class FlightImpApiController : ApiController
    {
        ...
        public HttpResponseMessage Index(string page, string pageSize, string code, string flightNo, string fda, string tda)
        {
            ...
            IList<Layer.Flight> flights = new DataAccess.FlightAccess().GetPagingMobile(pageInput,
                                                                                  pageSizeInput,
                                                                                  codeInput,
                                                                                  flightNoInput,
                                                                                  fromDate,
                                                                                  ...
                                                                                  ref total);
            ResultFlightImp result = new ResultFlightImp();
            result.total = total;
        ...
        public HttpResponseMessage GetAllFlight()
        {
            ResultFlightImp result = new ResultFlightImp();
            result.total = 0;
            result.Flights = new DataAccess.FlightAccess().GetAllFlight().ToList();
            return Request.CreateResponse(HttpStatusCode.OK, result);
